...
atoi_mod( tok )
{
	...
	tokString = _admin::strreplacer( tok, "lower" );
	tokID = _admin::atoi( tok );
	if ( !isDefined ( tokID ) )
		return undefined;
...
clean_string ( str ) {
	lower = _admin::strreplacer(str, "lower");
	mono = _admin::monotone( lower );
	return _admin::monotone( mono );
}
